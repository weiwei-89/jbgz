...
public class MqttDriver implements TcpDriver {
    ...
    public void init() throws Exception {
        this.connector = new DefaultMqttConnector(10*1000);
    }
    ...
    public MqttSession connect(Map<String, String> param) throws Exception {
        ...
        String clientId = param.get("client-id");
        if(StringUtils.isBlank(clientId)) {
            throw new Exception("client_id is blank");
        }
        String userName = param.get("user-name");
        ...
        this.connector.connect(config, user);
        MqttSession session = this.connector.getMqttSession(config, user);
